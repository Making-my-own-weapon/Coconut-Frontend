...
const GridCard: React.FC<{
  ...
  return (
    ...
    >
      {/* 왼쪽: 이름/온라인/메모 (반응형) */}
      <div className="flex flex-col items-center justify-center w-72 bg-slate-900/80 p-4 md:p-6 h-full min-w-0">
        <div className="flex flex-col items-center gap-2 w-full">
          <span className="text-white font-bold text-sm md:text-lg text-center w-full truncate">
            {name}
          </span>
          {isOnline && (
        ...
        <textarea
          className="w-full max-w-[280px] md:max-w-[320px] h-32 md:h-48 resize-none rounded-md bg-slate-800/80 text-slate-200 p-2 md:p-4 text-xs md:text-base mt-4 md:mt-6 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-inner"
      ...
      {/* 오른쪽: 문제 정보/진행률 (반응형) */}
      <div className="flex-1 flex flex-col justify-center p-2 md:p-4 gap-2 md:gap-4 h-full w-full min-w-0">
        <div className="flex flex-row items-start gap-4 md:gap-12 h-full">
          <div className="flex flex-col gap-3 md:gap-6 min-w-0 flex-1 justify-center h-full">
            {/* 풀고 있는 문제 */}
            <div className="min-w-0">
              <span className="text-slate-400 text-xs font-medium">풀고 있는 문제</span>
              <div className="text-white font-semibold text-xs md:text-base mt-1 truncate">
            ...
            <div className="min-w-0">
              ...
              <div className="flex gap-1 flex-wrap mt-1">
                {wrongProblemObjs.length === 0 ? (
                  ...
                  wrongProblemObjs.slice(0, 2).map((p) => (
                    <span
                      key={p.problemId}
                      className="px-1 md:px-2 py-0.5 rounded-full bg-red-500/20 text-red-400 text-xs font-semibold truncate max-w-[80px] md:max-w-[120px]"
                      title={p.title}
                    >
                      {p.title}
                    </span>
                ...
                {wrongProblemObjs.length > 2 && (
                  <span className="px-1 md:px-2 py-0.5 rounded-full bg-slate-600 text-slate-300 text-xs font-semibold">
                    +{wrongProblemObjs.length - 2}
                  </span>
                )}
              </div>
            ...
            {/* 진행률: 항상 보이도록 최소 크기 보장 */}
            <div className="flex flex-col items-start mt-2 md:mt-4 w-full min-w-0">
              ...
              <div className="relative w-full h-4 md:h-8 bg-slate-700 rounded-full overflow-hidden shadow-inner min-w-0">
                <div
